...
export class MessageBroker {
    ...
    public messageManager(socket: Socket, message: Message): void {
        switch (message.type) {
            case "doc":
                this.enterDoc(socket, message.data.docID);
                break;

            case "createDoc":
                this.createDoc(socket, message.data.docName);
                break;

            case "addText":
                this.sendTextAfterAddition(socket, {
                    wordPos: message.data.wordPos,
                    wordText: message.data.wordText,
                    additionPos: message.data.additionPos,
                    additionText: message.data.additionText,
                } as TextToAdd);
                break;

            case "addChar":
                this.addChar(socket, message.data.char, message.data.pos);
                break;

            case "rmChar":
                this.delChar(socket, message.data.pos);
                break;
    ...
    private enterDoc(socket: Socket, docId: number | undefined): void {
        try {
            if (!docId || docId <= 0) {
                throw new Error("Le numéro doit être supérieur ou égal à 0.");
            }

            //envoyer le contenu du document
            ...
            //envoyer les position des autres utilisateurs présents
    ...
    private createDoc(socket: Socket, docName: string | undefined): void {
        try {
            if (!docName) {
                docName = "Sans Titre";
            }
    ...
    private sendTextAfterAddition(socket: Socket, textToAdd: TextToAdd): void {
        try {
            if (
                textToAdd.wordPos == null ||
                textToAdd.wordText == null ||
                textToAdd.additionPos == null ||
                textToAdd.additionText == null
            ) {
                throw new Error("Une ou plusieurs propritétés nécéssaires à l'ajout d'un messages est vide.");
            }
    ...
    public addChar(socket: Socket, char: string | undefined, pos: number | undefined): void {
        try {
            if (!char || char.length !== 1) {
                throw new Error("Le param char doit être un seul caractère et non vide.");
            }
            if (pos == null) {
                throw new Error("Le param pos doit être renseigné.");
            }
    ...
    public delChar(socket: Socket, pos: number | undefined): void {
        try {
            if (!pos) {
                throw new Error("La position de suppression doit être renseignée et supérieure à 0.");
            }
